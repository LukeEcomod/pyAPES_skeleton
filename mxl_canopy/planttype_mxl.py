...
balance. Neglects rootzone processes and seasonal cycle; aimed for linking canopy
processes and convective mixed boundary layer (pyAPES-mxl)
...
Changes by SL 10.12.2019:
    - removed root zone and seasonal cycle models 
...
from canopy.planttype.photo import photo_c3_medlyn_farquhar
...
class PlantType(object):
    r""" Contains plant-specific properties and state variables 
    """

    def __init__(self, z, p, ctr, loc):
        r""" Initialises a planttype object using given parameters.
        ...
        # physical structure
        self.LAI = p['LAImax']  # maximum annual 1-sided LAI [m2m-2]
        ...
        self.photop['Vcmax'] = f * self.photop0['Vcmax']
        self.photop['Jmax'] =  f * self.photop0['Jmax']
        self.photop['Rd'] =  f * self.photop0['Rd']
    ...
    def update_daily(self, doy, T, PsiL=0.0, Rew=1.0):
        r""" Updates planttype gas-exchange parameters
...
#            self.lad = self.lad_normed * self.LAI
    ...
    def run(self, forcing, parameters, controls):
        ...
        # prepare outputs
        pt_stats, layer_stats = self._outputs(sl, sh, f_sl=parameters['sunlit_fraction'], df=parameters['dry_leaf_fraction'])
    ...
    def _outputs(self, sl, sh, f_sl, df):
        """
        Combines outputs.
        Args:
            sl, sh (dict)
